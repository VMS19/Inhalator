...
class IndicatorAlertBar(object):
    ...
    R_LETTER_PATH = os.path.join(RESOURCES_DIRECTORY,
                                    "r_letter.png")
    ...
    def __init__(self, parent, events, drivers, measurements):
        ...
        self.r_letter_image = PhotoImage(name="R",
                                         file=self.R_LETTER_PATH)
        ...
        record_sensors_image = None
        if self.configs.record_sensors:
            record_sensors_image = self.r_letter_image

        self.record_sensors = Label(master=self.system_info_frame,
                                    image=record_sensors_image,
                                    fg=Theme.active().ALERT_BAR_OK_TXT,
                                    bg=Theme.active().ALERT_BAR_OK)
    ...
    def textual(self):
        ...
        return [self.version_label, self.timestamp_label,
                self.message_label, self.battery_label, self.battery_icon, self.record_sensors]
